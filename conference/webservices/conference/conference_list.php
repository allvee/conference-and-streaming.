...
if($_SESSION['conference']['user_type']=="Super User") {
	...
	$query = "SELECT ID, Conf_Name, USER, Start_Time, End_Time, Participants, Recording, Notification_Channel, STATUS,room_number, weblink, Schedule_Conf FROM $tbl where UserOrg ='$org_ID' and `isDeleted` = 'No'";
} else {
    $query = "SELECT ID, Conf_Name, USER, Start_Time, End_Time, Participants, Recording, Notification_Channel, STATUS,room_number, weblink, Schedule_Conf FROM $tbl where USER ='$UserID' and `isDeleted` = 'No'";
}
