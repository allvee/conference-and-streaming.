...
function table_initialize_firewall_org_user() {
    ...
    if (permission.add_permission != 'yes') {
        ...
        $('#add_button').html('<div class="frmFldAcc  col-md-5"></div>' +
            ...
            '<button type="button" class="btn btn-primary btn-test" style="background-image:url(conference/img/add.png); margin-top: 7%; font-size: 17px; background-position: center center" onclick="show_firewall_org_user(); return false;">' +
            '</button> </div>' +
...
function organization_sync(){
    ...
    $.ajax({
        type: 'POST',
        url: "conference/webservices/conference/role_management/sync_organization.php",
        async: false,
