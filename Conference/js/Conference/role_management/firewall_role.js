...
function table_initialize_firewall_role() {
    ...
    if (permission.add_permission != 'yes') {
        ...
        $('#add_button').html('<div class="frmFldAcc  col-md-5"></div>' +
            ...
            '<button type="button" class="btn btn-primary btn-test" style="background-image:url(conference/img/add.png); margin-top: 7%; font-size: 17px; background-position: center center" onclick="show_firewall_role_form(); return false;">' +
            '</button> </div>' +
...
function load_menu_of_current_role(){
    ...
    report_role_menu();
    $("#dataTables_role_menu").after('<div class="row"> </div><div class="frmFldAcc col-md-3"></div>'+
...
function role_menu_sync() {
    $.ajax({
        type: 'POST',
        url: "conference/webservices/con/role_management/sync_role_menu.php",
        async: false,
        ...
        success: function (response) {
            ...
            if (res.status) {
                ...
                //display_content_custom("100", "#modalData");
                showUserMenu('firewall_role_menu_association');
