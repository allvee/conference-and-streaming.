...
function table_initialize_firewall_user_role_association() {
    ...
    $('#add_button').html('<div class="frmFldAcc  col-md-5"></div>' +
        ...
        '<button type="button" class="btn btn-primary btn-test" style="background-image:url(img/add.png); margin-top: 7%; font-size: 17px; background-position: center center" onclick="show_firewall_user_role_association_form(); return false;">' +
        '</button> </div>' +
...
function show_firewall_user_role_association_form() {
    write_activity_log('SHOW_USER_ROLE', 'SHOW_USER_ROLE', cms_url['activity_log']);
...
function edit_firewall_user_role_association(obj, action_id){
    ...
    var full_name = dataArray[0];
    var role_name = dataArray[1];
    $("#user_role_association_full_name option").filter(function() {
        return this.text == full_name;
    }).attr('selected', true);

    $("#user_role_association_role_name option").filter(function() {
        return this.text == role_name;
    }).attr('selected', true);


    $('#submit').html('Update');
    $("#user_role_association_full_name").trigger("chosen:updated");
    $("#user_role_association_role_name").trigger("chosen:updated");
