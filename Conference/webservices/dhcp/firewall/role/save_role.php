...
if($action == 'delete'){
    ...
    $user_data['component'] = 'DELETE_ROLE';
    $user_data['message'] = DELETE_ROLE . json_encode($_REQUEST);
    ...
    $user_data['user'] = $_SESSION['firewall']['login_id'];
    $user_data['component'] = 'UPDATE_ROLE';
    $user_data['message'] = UPDATE_ROLE . json_encode($_REQUEST);
    write_activity_log_data($cn, $user_data);
    ...
    $qry="UPDATE `$tbl` SET `NAME` = '$name' ,`org_id` = '$org_id',
    STATUS = 'active' ,
    last_updated = '$last_updated' WHERE `id` = '$action_id'";

} else{
    $user_data['user'] = $_SESSION['firewall']['login_id'];
    $user_data['component'] = 'ADD_ROLE';
    $user_data['message'] = ADD_ROLE . json_encode($_REQUEST);
    write_activity_log_data($cn, $user_data);

    $qry="insert into `$tbl` (NAME,org_id,STATUS,last_updated)";
    $qry .= " values ('$name','$org_id','active','$last_updated')";
