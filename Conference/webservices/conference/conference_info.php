...
else
{
    ...
    $end_date_time_split= explode(' ', $end_date_time);
    $minute= round(abs($end_date_time_split[1] - $date_time_split[1]) / 60,2);
    $slot=($minute+1)/30;
...
if ($is_error == 0) {
    $return_data = array('status' => true, 'Conf_id' => $conf_id,'slot'=>$slot,'$total_column'=>$total_column, 'room_number'=>$room_number,'qry' =>$query1,'query2'=>$query2,'conf_id' => $conf_id,'Name' => $demo_name, 'UserID' => $user_id , 'Long_Number'=>$long_code, 'Web_Link' => $web_link, 'Room_Number' => $room_number,
